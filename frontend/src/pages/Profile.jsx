...
const Profile = ({ profile, requests = [], refresh }) => {
  ...
  useEffect(() => {
    if (activeTab === 'friends') {
      api.get('/friendships').then(res => setFriends(res.data)).catch(() => setFriends([]));
    }
  }, [activeTab]);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[400px_1fr] gap-8 mt-8">
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-[#1e293b]/40 backdrop-blur-xl rounded-[3rem] p-10 flex flex-col items-center border border-slate-200 dark:border-slate-800/50 shadow-xl">
              ...
              <h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tighter mb-2">@{username}</h2>
              <span className="bg-[#facc15] text-slate-900 px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-lg">LVL {calculatedLevel}</span>
            </div>
            ...
            <div className="bg-white dark:bg-[#1e293b]/40 backdrop-blur-xl rounded-[2.5rem] p-8 border border-slate-200 dark:border-slate-800/50 shadow-xl">
              <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-6">Sosyal Etkileşim</h3>
              <div className="space-y-3 mb-6">
                {requests.map(req => (
                  <div key={req.id} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-800/40 rounded-xl">
                    <span className="font-bold text-xs text-slate-700 dark:text-slate-200">@{req.senderName}</span>
                    <button onClick={() => handleAction('accept', req.id)} className="w-8 h-8 bg-emerald-500 text-white rounded-lg flex items-center justify-center">✓</button>
                  </div>
                ))}
                {requests.length === 0 && <p className="text-[10px] text-slate-400 text-center py-2 italic">İstek yok.</p>}
              </div>
              <input 
                type="text" placeholder="Arkadaş ara..." 
          ...
          <div className="space-y-6">
            {activeTab === 'stats' ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <StatCard label="Mevcut Seri" value="0" icon="🔥" color="text-orange-500" />
                <StatCard label="En İyi Seri" value="0" icon="📈" color="text-indigo-500" />
              </div>
            ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {friends.map(friend => (
                    <div key={friend.id} className="flex items-center gap-4 p-5 bg-slate-50 dark:bg-slate-800/60 rounded-[2rem] border border-slate-100 dark:border-slate-700/30 transition-all hover:border-indigo-500/50">
                      <div className="w-12 h-12 bg-indigo-600 rounded-2xl flex items-center justify-center text-white font-black text-xl shadow-lg">{friend.username.charAt(0).toUpperCase()}</div>
                      <div>
                  ...
                  {friends.length === 0 && <p className="col-span-full text-center text-slate-400 py-20 font-black italic">Henüz hiç arkadaşın yok. Yeni birilerini aramaya ne dersin?</p>}
                </div>
              </div>
            )}
          </div>
