...
const Settings = ({ refresh }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      showMessage("error", error.response?.data?.message || "Kaydedilirken bir hata oluştu!");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const submitPasswordChange = async () => {
    ...
    try {
      ...
      showMessage("error", error.response?.data?.message || "Şifre güncellenemedi!");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const handleDeleteAccount = async () => {
    const isConfirmed = window.confirm("Hesabınızı kalıcı olarak silmek istediğinize emin misiniz? Bu işlem geri alınamaz!");
    ...
    try {
      ...
      showMessage("error", error.response?.data?.message || "Hesap silinirken hata oluştu!");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f172a] text-white p-4 md:p-8 font-sans relative overflow-x-hidden">
      {message.text && (
        <div className={`fixed top-8 right-8 px-6 py-3 rounded-xl font-medium shadow-2xl z-[9999] animate-fadeIn ${
            message.type === "success" ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/50" : "bg-red-500/20 text-red-400 border border-red-500/50"
          }`}>
          {message.text}
        </div>
      )}
      ...
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          <button onClick={() => navigate(-1)} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700 transition">←</button>
          <div>
              ...
              {tabs.map((tab) => (
                <button
                  ...
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all whitespace-nowrap ${
                    activeTab === tab.id ? "bg-indigo-500/20 text-indigo-400 font-medium border border-indigo-500/30" : "text-slate-400 hover:bg-slate-800/50 hover:text-slate-200 border border-transparent"
                  }`}
                >
            ...
            <div className="bg-[#1e293b]/40 border border-slate-800/80 rounded-3xl p-6 md:p-8 shadow-xl min-h-[500px] flex flex-col">
              ...
              {activeTab === "profile" && (
                <div className="animate-fadeIn">
                  ...
                  <div className="mb-8">
                    <label className="block text-sm font-bold text-slate-400 mb-3 uppercase tracking-widest text-[10px]">Kapak Fotoğrafı</label>
  ...
  />
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center transition-opacity cursor-pointer" onClick={() => bannerInputRef.current.click()}>
                        <span className="bg-white text-black px-4 py-2 rounded-lg text-xs font-bold shadow-xl">Kapağı Değiştir</span>
                      </div>
                  ...
                  <div className="flex flex-col md:flex-row items-center gap-6 mb-8 p-4 bg-slate-800/20 rounded-2xl border border-slate-700/30">
                    <div className="relative w-24 h-24 rounded-full border-4 border-slate-800 overflow-hidden shadow-2xl group">
                      {avatarPreview ? (
                        <img src={avatarPreview} alt="Önizleme" className="w-full h-full object-cover" />
                      ) : formData.avatar && formData.avatar !== "default-avatar.png" ? (
                        <img src={`http://localhost:3000/public/uploads/avatars/${formData.avatar}`} alt="Profil" className="w-full h-full object-cover" />
                      ) : (
                        <div className="w-full h-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-3xl font-black">{formData.name?.charAt(0)}</div>
                      )}
                    ...
                    <div className="text-center md:text-left">
                      <button onClick={() => fileInputRef.current.click()} className="px-4 py-2 bg-indigo-500/10 hover:bg-indigo-500/20 text-indigo-400 border border-indigo-500/30 rounded-xl text-sm font-bold transition-all mb-2">Profil Fotoğrafını Seç</button>
                      <p className="text-[10px] text-slate-500 uppercase tracking-widest">JPG, PNG veya WebP. Max 2MB.</p>
                  ...
                  <div className="space-y-5">
                    <div>
                      ...
                      <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-indigo-500 outline-none transition-all" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">E-posta (Değiştirilemez)</label>
                      <input type="email" value={formData.email} disabled className="w-full bg-slate-800/30 border border-slate-700/50 rounded-xl px-4 py-3 text-slate-500 cursor-not-allowed" />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-slate-400 mb-1.5">Unvan / Bio</label>
                      <input type="text" name="title" value={formData.title} onChange={handleChange} placeholder="Örn: Yazılım Geliştirici" className="w-full bg-slate-800/50 border border-slate-700 rounded-xl px-4 py-3 text-white focus:ring-1 focus:ring-indigo-500 outline-none transition-all" />
                    </div>
                  </div>
                </div>
              )}
                    ...
                    {['focusTime', 'shortBreak', 'longBreak'].map((field) => (
                      <div key={field}>
                        <div className="flex justify-between mb-2">
                          <label className="font-medium text-slate-300 capitalize">{field.replace(/([A-Z])/g, ' $1')}</label>
                          <span className="text-indigo-400 font-bold">{formData[field]} dk</span>
                        </div>
                        <input type="range" name={field} min={field === 'focusTime' ? "15" : "3"} max={field === 'focusTime' ? "60" : "30"} step={field === 'shortBreak' ? "1" : "5"} value={formData[field]} onChange={handleChange} className="w-full h-2 bg-slate-700 rounded-lg appearance-none cursor-pointer accent-indigo-500" />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {activeTab !== "account" && (
                <div className="mt-auto pt-8 flex justify-end">
                  <button onClick={handleSave} disabled={isSaving} className="px-8 py-3 bg-indigo-600 hover:bg-indigo-700 disabled:opacity-50 rounded-xl font-bold transition-all flex items-center gap-2 shadow-lg shadow-indigo-500/20">
