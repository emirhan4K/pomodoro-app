    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center p-6 relative overflow-hidden transition-colors duration-500">
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl p-10 rounded-[2.5rem] shadow-2xl dark:shadow-indigo-900/20 border border-white dark:border-slate-800 w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-gradient-to-tr from-indigo-600 to-purple-600 rounded-2xl flex items-center justify-center text-white font-black text-3xl shadow-lg mx-auto mb-4">P</div>
          <h2 className="text-3xl font-black text-slate-800 dark:text-white tracking-tight">Tekrar Hoş Geldin</h2>
          <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium">Odaklanmaya hazır mısın?</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">E-posta</label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="block text-xs font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 ml-1">Şifre</label>
            <input
              type="password"
              value={password}
        ...
        <p className="mt-8 text-center text-sm text-slate-600 dark:text-slate-400 font-medium">
          Henüz hesabın yok mu? <Link to="/register" className="text-indigo-600 dark:text-indigo-400 font-bold hover:underline ml-1">Kayıt Ol</Link>
        </p>
      </div>
    </div>
  );
