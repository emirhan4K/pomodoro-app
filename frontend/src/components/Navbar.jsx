...
const Navbar = () => {
  ...
  useEffect(() => {
    if (!isSearchOpen) { setResults([]); return; }
  ...
  const getNotificationIcon = (type) => {
    switch(type) {
      case 'FRIEND_REQUEST': return <span className="text-xl">🫂</span>;
      case 'LEVEL_UP': return <span className="text-xl">🏆</span>;
      case 'SYSTEM': return <span className="text-xl">💻</span>;
  ...
  return (
    <nav className="bg-white dark:bg-[#1e293b]/60 backdrop-blur-md px-4 md:px-8 py-4 flex justify-between items-center rounded-b-[2rem] border-b border-slate-100 dark:border-slate-800 shadow-lg relative z-[100]">
      ...
      <div className="flex items-center gap-2 md:gap-3 relative">
        ...
        <div ref={searchContainerRef} className={`absolute top-0 right-32 transition-all duration-300 ease-out ${isSearchOpen ? 'w-80 opacity-100' : 'w-0 opacity-0 pointer-events-none'}`}>
          <div className="relative group">
            ...
            />
            {loading ? (
              <div className="absolute inset-y-0 right-4 flex items-center"><div className="w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div></div>
            ) : (
              <div className="absolute inset-y-0 right-4 flex items-center pointer-events-none"><span className="text-slate-400">🔍</span></div>
            )}
          </div>
          ...
          {isSearchOpen && searchTerm.trim().length >= 2 && (
            <div className="absolute top-full left-0 right-0 mt-3 bg-white/95 dark:bg-[#0f172a]/95 border border-slate-200 dark:border-slate-700/50 rounded-[1.5rem] shadow-2xl overflow-hidden backdrop-blur-xl animate-in fade-in slide-in-from-top-2 duration-300 z-[110]">
              <div className="p-2 max-h-[350px] overflow-y-auto scrollbar-hide">
                {results.length > 0 ? (
                  results.map((user) => (
                    <div key={user.userId} onClick={() => { navigate(`/profile/${user.userId}`); setIsSearchOpen(false); setSearchTerm(""); }} className="flex items-center gap-3 p-3 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-xl cursor-pointer transition-all group">
                      <div className="w-10 h-10 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden border border-slate-200 dark:border-slate-600 group-hover:border-indigo-500/50 transition-all flex-shrink-0">
                        {user.avatar && user.avatar !== "default-avatar.png" ? ( <img src={user.avatar.startsWith('http') ? user.avatar : `https://pomodoro-app-omxg.onrender.com/public/uploads/avatars/${user.avatar}`} className="w-full h-full object-cover" alt="" /> ) : ( <div className="w-full h-full flex items-center justify-center font-bold text-slate-500">{user.username[0].toUpperCase()}</div> )}
                      </div>
                ...
                ) : ( <div className="py-8 text-center"><p className="text-xs text-slate-500 dark:text-slate-400 italic">Sonuç bulunamadı 😕</p></div> )}
              </div>
            </div>
          )}
        </div>
          ...
          {isNotifDropdownOpen && (
            <div className="absolute top-14 right-0 w-80 sm:w-96 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-2xl rounded-[2rem] shadow-2xl border border-slate-200/80 dark:border-slate-700/50 overflow-hidden z-[110] transform transition-all animate-in fade-in slide-in-from-top-4 duration-200 origin-top-right flex flex-col max-h-[400px]">
              <div className="p-4 border-b border-slate-100 dark:border-slate-800/80 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/20">
                <h3 className="font-black text-slate-800 dark:text-white uppercase tracking-widest text-xs">Bildirimler</h3>
                {unreadCount > 0 && <span className="text-[10px] font-bold text-indigo-500 bg-indigo-500/10 px-2 py-1 rounded-md">{unreadCount} Okunmamış</span>}
              </div>
                  ...
                  notifications.map((notif) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 pt-1">
                        <p className={`text-sm leading-tight break-words ${notif.isRead ? 'text-slate-600 dark:text-slate-400 font-medium' : 'text-slate-800 dark:text-slate-200 font-bold'}`}>
                          {notif.content}
                        </p>
                      </div>
                      {!notif.isRead && <div className="w-2.5 h-2.5 bg-indigo-500 rounded-full flex-shrink-0 mt-1.5 shadow-md shadow-indigo-500/50"></div>}
                    </div>
                  ))
                ) : (
                  <div className="py-12 flex flex-col items-center justify-center text-center px-4">
                    <span className="text-4xl mb-3 opacity-50 text-slate-400">📭</span>
                    <p className="text-xs text-slate-500 dark:text-slate-400 font-bold uppercase tracking-widest">Henüz bildirim yok</p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">Burasi oldukça sessiz görünüyor.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          >
            <div className="w-9 h-9 bg-[#0f172a] rounded-xl flex items-center justify-center text-white font-black shadow-inner border border-slate-600 overflow-hidden">
              {profile?.avatar && profile.avatar !== 'default-avatar.png' ? (
                <img src={profile.avatar?.startsWith('http') ? profile.avatar : `https://pomodoro-app-omxg.onrender.com/public/uploads/avatars/${profile.avatar}`} className="w-full h-full object-cover" alt="Avatar"/>
            ...
            <div className="absolute top-14 right-0 w-80 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-2xl rounded-[2.5rem] shadow-2xl border border-slate-200/80 dark:border-slate-700/50 overflow-hidden z-[110] transform transition-all animate-in fade-in slide-in-from-top-4 duration-200 origin-top-right">
              ...
              <div className="relative h-32 w-full overflow-hidden">
                {profile?.banner && profile.banner !== 'default-banner.png' ? (
                  <img src={profile.banner?.startsWith('http') ? profile.banner : `https://pomodoro-app-omxg.onrender.com/public/uploads/banners/${profile.banner}`} className="w-full h-full object-cover" alt="Banner"/>
              ...
              <div className="p-5 pt-4 border-b border-slate-100 dark:border-slate-800/80">
                <div className="w-full">
                  ...
                  <div className="w-full h-2 bg-slate-200 dark:bg-slate-700/50 rounded-full overflow-hidden shadow-inner">
                    <div className="h-full bg-gradient-to-r from-indigo-500 to-purple-500 rounded-full transition-all duration-1000 shadow-lg" style={{ width: `${progressPercentage}%` }}></div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3 space-y-1">
                <button onClick={() => { navigate('/profile?tab=stats'); setIsDropdownOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-xl transition-all group">
                  <span className="text-lg group-hover:scale-110 group-hover:-rotate-12 transition-transform">👤</span> Profilim
                </button>
                <button onClick={() => { navigate('/statistics'); setIsDropdownOpen(false); }} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-xl transition-all group">
                  <span className="text-lg group-hover:scale-110 group-hover:rotate-12 transition-transform">📊</span> İstatistikler
                </button>
                <button onClick={() => { navigate('/profile?tab=friends'); setIsDropdownOpen(false); }} className="w-full flex items-center justify-between px-3 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 hover:text-indigo-600 dark:hover:text-indigo-400 rounded-xl transition-all group">
                  <div className="flex items-center gap-3"><span className="text-lg group-hover:scale-110 group-hover:rotate-12 transition-transform">🫂</span> Arkadaşlarım</div>
                </button>
                ...
                <button className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800/60 rounded-xl transition-all group" onClick={() => { navigate('/settings?tab=profile'); setIsDropdownOpen(false); }} >
                  <span className="text-lg group-hover:rotate-45 transition-transform duration-300">⚙️</span> Ayarlar
                </button>
              </div>
              ...
              <div className="p-3 border-t border-slate-100 dark:border-slate-800/60 bg-slate-50/50 dark:bg-slate-800/20">
                <button onClick={handleLogout} className="w-full flex items-center gap-3 px-3 py-2.5 text-sm font-bold text-rose-500 hover:bg-rose-100 dark:hover:bg-rose-500/10 rounded-xl transition-all group">
                  <span className="text-lg group-hover:translate-x-1 transition-transform">🚪</span> Çıkış Yap
                </button>
              </div>
